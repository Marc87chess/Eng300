...
void LCDWriteChar(char c) {
    LCDWrite((c >> 4) & 0x0F, 1, 0, 1);  // Send upper nibble
    LCDWrite(c & 0x0F, 1, 0, 1);         // Send lower nibble
}
void LCDWriteString(char *str) {
    while (*str) {  // Loop through each character in the string till we hit NULL
        LCDWriteChar(*str);
...
int main(void) {
    ...
    sei(); // global interupts
    
    ///  SET MY REGISTERS
    PORTA.DIRSET |= 0b00100001;
    PORTA.DIRSET &= 0b00100001;
    EVSYS.CHANNEL0 = 0x41;
    ...
    while (1) {
       ...
       //METAL
       if (value[1] > 1000){// Make sure the Value isn't junk, sometimes the readings are randomly off
       if (value[1] >mthresh){
       ...
       // BUTTONS
       if (!(PORTA.IN &0b10000000))wasdown = 1;
       if ((PORTA.IN &0b10000000)&& wasdown){
           mthresh =  mthresh -2;
           wasdown = 0;
           c1--;
       }
       if (!(PORTA.IN &0b01000000))wasdown2 = 1;
       if ((PORTA.IN &0b01000000)&& wasdown2){
           mthresh = mthresh + 2;
           wasdown2 = 0;
           c1++;
       }
       
        if (((PORTA.IN &0b01000000)&&(PORTA.IN & 0b10000000))&& wasdown2){
            debug =! debug;
